...
object QueryProcessorAQP {
  ...
  def loadSparkSession() = {
    ...
    val spark = SparkSession.builder.appName("QueryProcessorAQP")
  ...
  def main(args: Array[String]): Unit = {
    ...
    fileList.foreach(queryFile => {
      ...
      //var maxLevles = partitions.map{parts => parts._2.map(x=>x.replace("lv","").toInt)}.map(p => p.reduceLeft(_ max _)) //take the max level of each predicate
      //var maxLevel = maxLevles.reduceLeft(_ max _) //found the max level betweeen ala the predicates

      var numOfLevels = partitions.map(parts => parts._2.length) //take the length of each Level Array
      var maxLevel = numOfLevels.reduceLeft(_ max _) //
      println("--> max num of Levels: " + maxLevel)

      for(count <- 1 to maxLevel){
        val resultFile = new File(resultPath + "results" + "_" + tpNum + "_" + "bal" + ".txt")
        val resultWriter = new FileWriter(resultFile, true) //appends


        var partsAQP = AQP(partitions, count)
        val (executionTime, loadTime, loadDataSize, result) = executeQuery(queryMap, partsAQP, dataset, queryName.dropRight(7)) //swdf: dropRight(4)

        resultWriter.append("Query: " + queryFile.getName + "_AQP_"  + count + "\n")
        ...
        //resultWriter.append("partitions: " + partsAQP.foreach(p => println(p._1 + ":" + p._2.mkString(","))) + "\n")
        resultWriter.append("partitions: ")
        partsAQP.foreach(p => resultWriter.append(p._1 + ":" + p._2.mkString(",") + " ## "))
        resultWriter.append("\n")
        //.append("max level: " + maxLevel)
  ...
  def AQP(partitions:Map[String, Array[String]], count: Integer): Map[String, Array[String]] = {
    var partsAQP = partitions.map{case(partition) => { (partition._1, partition._2.take(count))}}
